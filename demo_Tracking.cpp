///========================================================
//   Ref:
//   http://nashruddin.com/eyetracking-track-user-eye.html
///========================================================

#include <stdio.h>
#include "cv.h"
#include "highgui.h"

/*
#define  TPL_WIDTH       46      // template width      
#define  TPL_HEIGHT      46      // template height     
#define  WINDOW_WIDTH    48      // search window width 
#define  WINDOW_HEIGHT   48      // search window height
*/

//=============================
// Parametros para el rostro
//=============================
#define  TPL_WIDTH       245      // template width      
#define  TPL_HEIGHT      245      // template height     
#define  WINDOW_WIDTH    252      // search window width 
#define  WINDOW_HEIGHT   252      // search window height

#define  THRESHOLD       0.3
 
IplImage *frame, *tpl, *tm;
IplImage *imgOjo;
int      object_x0, object_y0, is_tracking = 0;
 
void mouseHandler( int event, int x, int y, int flags, void *param );
void trackObject(void);
 
// main code 
int main( void )
{
	
    CvCapture   *capture;
    int         key=1;
 
    /* initialize camera */
    capture = cvCaptureFromCAM( 0 );
    cvSetCaptureProperty( capture,CV_CAP_PROP_FRAME_WIDTH, 2*320 );
    cvSetCaptureProperty( capture,CV_CAP_PROP_FRAME_HEIGHT,2*240 );
 
    // always check 
    if( !capture ) 
		return 1;
 
    // get video properties, needed by template image 
    frame = cvQueryFrame( capture );
    if ( !frame ) 
		return 1;
    
    // create template image 
    tpl = cvCreateImage( cvSize( TPL_WIDTH, TPL_HEIGHT ), 
                         frame->depth, frame->nChannels );
    
    // create image for template matching result 
    tm = cvCreateImage( cvSize( WINDOW_WIDTH  - TPL_WIDTH  + 1,
                                WINDOW_HEIGHT - TPL_HEIGHT + 1 ),
                                IPL_DEPTH_32F, 1 );

    imgOjo = cvCreateImage(cvSize(50,50),8,3);

    // create a window and install mouse handler 
    cvNamedWindow( "video", CV_WINDOW_AUTOSIZE );
    cvNamedWindow( "ojo", 1*CV_WINDOW_AUTOSIZE );
    cvSetMouseCallback( "video", mouseHandler, NULL );
    
    while( key != 'q' ) 
	{
        // get a frame 
        frame = cvQueryFrame( capture );
 
        // always check 
        if( !frame ) 
			break;
 
        // 'fix' frame 
        //cvFlip( frame, frame, -1 );
        frame->origin = 0;
        
        /* perform tracking if template is available */
        if( is_tracking ) 
			trackObject();
        
        // display frame 
        cvShowImage( "video", frame );
		cvShowImage( "ojo", imgOjo );
 
        // exit if user press 'q' 
        key = cvWaitKey( 1 );

	if(key==27)
		break;
    }
 
    // free memory 
    cvDestroyWindow( "video" );
    cvReleaseCapture( &capture );
    cvReleaseImage( &tpl );
    cvReleaseImage( &tm );
	cvReleaseImage( &imgOjo );
    
    return 0;
}
 
// mouse handler 
void mouseHandler( int event, int x, int y, int flags, void *param )
{
    // user clicked the image, save subimage as template 
    if( event == CV_EVENT_LBUTTONUP )
	{
        object_x0 = x - ( TPL_WIDTH  / 2 );
        object_y0 = y - ( TPL_HEIGHT / 2 ); 
        
        cvSetImageROI( frame, cvRect( object_x0, object_y0, TPL_WIDTH, TPL_HEIGHT ) );
        cvCopy( frame, tpl, NULL );
        cvResetImageROI( frame );
 
        /* template is available, start tracking! */
        fprintf( stdout, "Template selected. Start tracking... \n" );
        is_tracking = 1;
    }
}
 
// track object 
void trackObject(void)
{
    CvPoint minloc, maxloc;
    double  minval, maxval;
 
    // setup position of search window 
    int win_x0 = object_x0 - ( ( WINDOW_WIDTH  - TPL_WIDTH  ) / 2 );
    int win_y0 = object_y0 - ( ( WINDOW_HEIGHT - TPL_HEIGHT ) / 2 );
    
	if( win_x0<0)
			win_x0 = 0;
		
	if (win_x0 > frame->width - WINDOW_WIDTH) 
		win_x0 = frame->width - WINDOW_WIDTH;
		
	if( win_y0 < 0 )
		win_y0 = 0;

	 if(win_y0 > frame->height - WINDOW_HEIGHT)
		win_y0 = frame->height - WINDOW_HEIGHT;

    /*
     * Ooops, some bugs here.
     * If the search window exceed the frame boundaries,
     * it will trigger errors.
     *
     * Add some code to make sure that the search window 
     * is still within the frame.
     */
    
    // search object in search window 
    cvSetImageROI( frame, 
                   cvRect( win_x0, 
                           win_y0, 
                           WINDOW_WIDTH, 
                           WINDOW_HEIGHT ) );	

    cvMatchTemplate( frame, tpl, tm, CV_TM_SQDIFF_NORMED );
    cvMinMaxLoc( tm, &minval, &maxval, &minloc, &maxloc, 0 );
    cvResetImageROI( frame );
    
    // if object found...     
	if( minval <= THRESHOLD/2 ) 
	{
        // save object's current location 
        object_x0 = win_x0 + minloc.x;
        object_y0 = win_y0 + minloc.y;
 
		CvRect box = cvRect(object_x0, object_y0,TPL_WIDTH,TPL_HEIGHT);

		char texto_correl[30];
		CvFont font = cvFont(1,1);

		// Valor de la correlacion
		sprintf(texto_correl,"TM: %0.3f",minval);		
		cvPutText(frame,texto_correl,cvPoint(object_x0, object_y0-5),&font,CV_RGB(255,0,0));

		// Posicion del ojo
		sprintf(texto_correl,"Pos_Y: %d",object_y0+TPL_HEIGHT/2);
		cvPutText(frame,texto_correl,cvPoint(object_x0,object_y0+TPL_HEIGHT+15),&font,CV_RGB(80,255,120));

		cvSetImageROI(frame,box);
		cvResize(frame,imgOjo);
		cvResetImageROI(frame);

        // and draw a box there 
        cvRectangle( frame,
                     cvPoint( object_x0, object_y0 ),
                     cvPoint( object_x0 + TPL_WIDTH, object_y0 + TPL_HEIGHT ),
                     cvScalar( 0, 0, 255, 0 ), 1, 0, 0 );
    } 
	else 
	{
		printf("Valor de la correlacion: %f\n",minval);
        // if not found... 
		cvZero(imgOjo);
        fprintf( stdout, "Lost object.\n" );
        is_tracking = 0;
    }
}
